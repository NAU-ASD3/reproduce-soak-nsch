...
for(year.do in year.do.vec){
  year.dta <- sub("_topical.do", "e_topical.dta", year.do)
  ...
  half.names <- setdiff(names(prop.NA)[prop.NA<0.1], c(
    "y","k2q35a","survey_year","year",
    "sc_aian",
    "sc_asian",
    "sc_nhpi"))
...
fwrite(out.dt, "data_Classif/NSCH_autism_new_reproduced.csv")

old.dt <- fread("data_Classif/NSCH_autism.csv")
setdiff(names(old.dt),names(out.dt))
dput(setdiff(names(out.dt),names(old.dt)))
sapply(list(old=old.dt,new=out.dt), dim)
all.equal(old.dt,out.dt[, names(old.dt), with=FALSE])
