...
** Step 0 install software, clone this repo

First install the most recently released version of R (it may work with older versions, but the most recently released version is your best bet, because all the packages are checked using this version).
I used R-4.5.2. Also install/update these packages:

#+begin_src R
  install.packages(c("data.table","mlr3resampling","mlr3learners","mlr3batchmark","glmnet","batchtools","ggplot2","nc"))
  update.packages(ask=FALSE)
#+end_src

And install git so you can clone this repo (or just download and unpack [[https://github.com/NAU-ASD3/reproduce-soak-nsch/archive/refs/heads/main.zip][the zip file]]).

On a cluster system, you typically want to store this repo in your project space.
...
On your laptop, you don’t need to create a scratch symlink (a scratch sub-directory will be created for you).
...
The figure below shows timings to re-compute the 60 train/test splits on various computers.

[[file:results/2026-02-24/figure-timings.png]]

On a fast 2025 super-computer (Alliance Canada rorqual)
- on which all 60 jobs run in parallel, wall time ~5 minutes.
- running in sequence on 1 CPU, wall time 2–3 hours.

On a 2025 Ubuntu laptop
- in parallel on 14 cores, wall time ~1 hour.
- in sequence on 1 CPU, wall time ~11 hours.

On a ~2018 Windows desktop (intel core i5 9th Gen),
- in parallel on 6 cores, wall time ~90 minutes.
- in sequence on 1 CPU, wall time 7–8 hours.
...
Then copy CSV files (and R analysis scripts) to a sub-directory of results (named using the current date).
The reproduced results files are named =NSCH_*.csv= in your results sub-directory.
You can execute the relevant lines of [[file:results_copy.R]], or just do the copy yourself (cp or gui).

For example, 

* the results @tdhock computed are in [[file:results/2026-02-24/]]
* the results @naalu computed are in [[file:results/2026-03-06/]]

** Step 4 Result figures

Now start R in your results sub-directory and run =figure-compare-paper.R=
which should make several figures that compare the original and
reproduced results (using the first =NSCH_*.csv= file in your results
directory).

For example, I ran [[file:results/2026-02-24/figure-compare-paper.R]], and got these result figures.
- [[file:results/2026-02-24/figure-compare-paper-reproduce-figure4.png]]
- [[file:results/2026-02-24/figure-compare-paper-reproduce-figure5.png]]
- [[file:results/2026-02-24/figure-compare-paper-pvalues.png]]
- [[file:results/2026-02-24/figure-compare-paper-AUC.png]]
- [[file:results/2026-02-24/figure-compare-paper-Accuracy.png]]
- [[file:results/2026-02-24/figure-compare-paper-Percent_error.png]]

If you want, you can run these additional analyses:

- [[file:results/2026-02-24/figure-compare-computation.R]] makes a [[file:results/2026-02-24/figure-compare-computation.png][figure]] to compare the results from the different computational methods.
- [[file:results/2026-02-24/figure-timings.R]] makes a [[file:results/2026-02-24/figure-timings.png][figure]] to compare the computation time of the different computational methods.

* Reproducing data generation

[[file:data_Classif_NSCH_autism_new.R]] has code for downloading Stata files
...
If we exclude three variables (sc_aian, sc_asian, sc_nhpi), as in [[file:data_Classif_NSCH_autism_new_reproduced.R]], we get consistent results.
